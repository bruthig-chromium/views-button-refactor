...
def getSignatureFor(src_file, method):
    ...
    url = url.format(file_name=urllib.parse.quote(bytes(src_file, 'utf-8'), safe=b''))
...
def getXrefsFor(signature):
    ...
    for file_result in search_results:
        ...
        for match in file_result['match']:
            ...
            elif match['type'] == 'EXTENDED_BY':
                xrefs.setdefault('extended_by', [])
                xrefs['extended_by'].append(getRefForMatch(filename, match))
            elif match['type'] == 'EXTENDS':
                xrefs['extends'] = getRefForMatch(filename, match)
    return xrefs
...
if __name__ == "__main__":
  ...
  hierarchy = {}
  getInheritanceHierarchy(signature, hierarchy)
  # print(hierarchy)
  # print(getSpreadsheetData(hierarchy))
  # print(json.dumps(hierarchy))
  print(getGraphviz(hierarchy))
